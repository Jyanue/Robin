...
EFI_SIMPLE_FILE_SYSTEM_PROTOCOL *gSimpleFileSystem;  //操作FAT文件系统的句�?
EFI_FILE_PROTOCOL *gFileRoot;		//操作文件的句�?

//================  Member function =======================================
//Name:InintGloabalProtocols
//Input:
//Output: 0: 所有protocol都�??获取
//				非零: 所对应位标志的protocol没有�?正确获取
//        
//Descriptor:
...
EFI_STATUS LocateFileRoot(void)
{
	EFI_STATUS  Status = 0;
	 ...
	 Status = gBS->LocateProtocol(
            ...
            NULL,
            (VOID**)&gSimpleFileSystem //实际程序�?用不到，还是预留了这�?接口 2019-06-10 19:51:02 luobing
    );
    if (EFI_ERROR(Status)) {
