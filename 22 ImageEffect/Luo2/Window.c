...
VOID ReviseImage(IN UINTN x,IN UINTN y,IN UINTN Width,IN UINTN Height,UINT8 Flag)
{
	...
	if(Flag & 0x1)  //上下颠倒
	{
		...
		CopyMem(BltBufferSrc,BltBufferDst,BltBufferSize);
	}
...
//Output: 
//Description: 螺旋显示清除
VOID SpiralShowClear(IN UINTN x, IN UINTN y, IN UINTN Width, IN UINTN Height, UINTN maxX, UINTN maxY,IN EFI_GRAPHICS_OUTPUT_BLT_PIXEL *color)
{
	INTN i;

	for (i = (INTN)((Width - 1) >> 1); i < (INTN)(Width - 1); i++)
		Line(i, 0, (Width - 1) >> 1, (Height - 1) >> 1, maxX, maxY, color);
	for (i = 0; i < (INTN)(Height - 1); i++)
		Line(Width - 1, i, (Width - 1) >> 1, (Height - 1) >> 1, maxX, maxY, color);
	for (i = (INTN)(Width - 1); i > 0; i--)
		Line(i, Height - 1, (Width - 1) >> 1, (Height - 1) >> 1, maxX, maxY, color);
	for (i = (INTN)(Height - 1); i > 0; i--)
		Line(0, i, (Width - 1) >> 1, (Height - 1) >> 1, maxX, maxY, color);
