...
VOID Line(UINTN x1, UINTN y1, UINTN x2, UINTN y2, UINTN maxX, UINTN maxY, EFI_GRAPHICS_OUTPUT_BLT_PIXEL *color)
{
	/*使用Bresenham算法*/
					  ...
					  IN EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ImageBuffer)
{
	...
	if(distanceY>Height-1)	//超过图像范围了，不显示
		return;
	if((distanceX+partWidth) > (Width-1))
	...
	//最后一个参数要注意
	gGraphicsOutput->Blt (gGraphicsOutput,ImageBuffer,EfiBltBufferToVideo,distanceX,distanceY,x1+distanceX,y1+distanceY,
					  ...
					  IN EFI_GRAPHICS_OUTPUT_BLT_PIXEL *ImageBuffer)
{
	...
	BltBufferSize = realPartHeight*realPartWidth*sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL);
  	gBS->AllocatePool (
...
//Input: x,y,Width,Height,imgX,imgY,ImageBuffer
//Output: Void
