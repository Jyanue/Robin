...
//============================ 鼠标图案在此处 begin ============================
#define gMousePic_Width   18 //pcx图像的宽度 
#define gMousePic_Height  25 //pcx图像的高度 
...
//Output: None
//Description: 显示鼠标图案
UINT16 xres=0;		//鼠标全局位置，只有鼠标显示函数可以使用
UINT16 yres=0;
...
VOID putMouseArrow(UINTN x,UINTN y)
{
	...
	if(x>=(SY_SCREEN_WIDTH-1-gMousePic_Width))
		x=SY_SCREEN_WIDTH-1-gMousePic_Width;
	if(y>=SY_SCREEN_HEIGHT-1-gMousePic_Height)
		y=SY_SCREEN_HEIGHT-1-gMousePic_Height;
	//0 还原鼠标覆盖的区域
	putRectImage(xres,yres,gMousePic_Width,gMousePic_Height,oldMouseHideZone);
	xres=(UINT16)x;
	yres=(UINT16)y;
	//1 保存当前鼠标要覆盖的区域
...
VOID initMouseArrow(VOID)
{
	xres=0;
	yres=0;
	getRectImage(0,0,gMousePic_Width,gMousePic_Height,oldMouseHideZone);	//存储鼠标覆盖区域
	putPCX256(0,0,gMousePic_Width,gMousePic_Height,gMousePicColorTable,gMousePicPicture,1);	//maskcolor=1
}
