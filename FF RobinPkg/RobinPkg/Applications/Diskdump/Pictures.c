//code by luobing 2019-5-18 16:20:09
//lbdebug begin
//#include "Font.h"
//#include "Keyboard.h"
//lbdebug end
  ...
  ) 
{
  ...
  Status = LoadFile(BmpFilePath, (VOID **)&ImageData, (UINT32 *)&ImageSize);
...
// EFI_STATUS move (IN CONST EFI_GUID *LogoFile) 
EFI_STATUS move (VOID) 
{
  EFI_INPUT_KEY Key;
  UINTN index;
  UINTN XCoordinate;
  ...
  XCoordinate = (gGraphicsOutput->Mode->Info->HorizontalResolution / 2) - (mWidth / 2);
  YCoordinate = (gGraphicsOutput->Mode->Info->VerticalResolution / 2) - (mHeight / 2);
  ...
  do {
    ...
    if (Key.ScanCode != SCAN_NULL) {
      if (Key.ScanCode == SCAN_UP) {
        ...
        NewX = XCoordinate;
        NewY = YCoordinate + 5;
        if (NewY > gGraphicsOutput->Mode->Info->VerticalResolution - mHeight) {
          NewY = gGraphicsOutput->Mode->Info->VerticalResolution - mHeight;
        }
      } else if (Key.ScanCode == SCAN_RIGHT) {
        NewY = YCoordinate;
        NewX = XCoordinate + 5;
        if (NewX > gGraphicsOutput->Mode->Info->HorizontalResolution - mWidth) {
