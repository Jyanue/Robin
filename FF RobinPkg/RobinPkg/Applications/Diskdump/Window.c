...
//Function Name: MaskingTransparent
//Input: x1,y1,Width,Height,*ImagePointer,*ImageOverlayArea,Alpha
//Output:
//Description: ImagePointer,指向Width*Height大小的图像缓冲区，其大小必定为Width*Height*sizeof(EFI_GRAPHICS_OUTPUT_BLT_PIXEL)
//						 ImageOverlayArea指向要被覆盖的图像,并且作为处理后的图像内存输出
//实现透明效果，使用Alpha方式，Alpha值可调,从0-16可调
//2013-5-20 22:00:48 luobing 为了通用，修改为对两段图像内存进行处理，提取和显示程序员自己实现。
