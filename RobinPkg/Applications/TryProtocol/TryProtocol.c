...
EFI_STATUS ListProtocolMsg(EFI_GUID *Protool);
/***
  ...
  )
{
	...
	WaitKey();
  {
    ...
    //List serialIO
    gST->ConOut->SetAttribute(gST->ConOut,EFI_BACKGROUND_BLACK|EFI_RED);  //UEFI SPEC 2.8 Page451
    Print((const CHAR16*)L"Action: 寻找SerialIoProtocol\n");
    ListProtocolMsg(&gEfiSerialIoProtocolGuid);
     gST->ConOut->SetAttribute(gST->ConOut,EFI_BACKGROUND_BLACK|EFI_CYAN);  //UEFI SPEC 2.8 Page451
     Print((const CHAR16*)L"Action: 寻找RngProtocol\n");
    ListProtocolMsg(&gEfiRngProtocolGuid);
    gST->ConOut->SetAttribute(gST->ConOut,EFI_BACKGROUND_BLACK|EFI_LIGHTGRAY); 
...
EFI_STATUS ListProtocolMsg(EFI_GUID *Protool)
{
  ...
  //get the handles which supports 
	...
	//list message
	Print((const CHAR16*)L"=== Protocol信息获取  Begin ===\n");
  Print((const CHAR16*)L" GUID: {0x%08x, 0x%04x, 0x%04x, {",Protool->Data1,Protool->Data2,Protool->Data3);
  for (i = 0; i < 8; i++)
    Print((const CHAR16 *)L" 0x%02x", Protool->Data4[i]);
  Print((const CHAR16 *)L"}}\n");
  if (EFI_ERROR(Status))
  {
    Print((const CHAR16 *)L"没有找到相应的Protocol!\n");
      }
  else
  {
    Print((const CHAR16 *)L"找到Handle: %d个\n", HandleCount);
  }
  Print((const CHAR16*)L"=== Protocol信息获取  END ===\n");
  if(myHandleBuff!=NULL)
    FreePool(myHandleBuff);
  return Status;
}