...
EFI_STATUS InitTcp4SocketFd(INTN index)
{
    ...
    //3 Create Transmit data
    Status = gBS->CreateEvent(EVT_NOTIFY_WAIT, TPL_CALLBACK, (EFI_EVENT_NOTIFY)NopNoify , (VOID*)&CurSocket->SendToken, &CurSocket->SendToken.CompletionToken.Event);
    if(EFI_ERROR(Status)) 
    {
...
EFI_STATUS SendTCP4Socket(UINTN index, CHAR8* Data, UINTN Lenth)
{
    ...
    if(EFI_ERROR(Status))
    {
        gST->ConOut->OutputString(gST->ConOut,L"Send: WaitForEvent fail!\n\r");
        Print(L"Status=%x\n", Status);
    }
...
EFI_STATUS RecvTCP4Socket(IN UINTN index, IN CHAR8* Buffer, IN UINTN Length, OUT UINTN *recvLength)
{
    ...
    // Print(L"Recv: %d bytes\n", CurSocket->m_RecvData->DataLength);
    *recvLength = CurSocket->m_RecvData->DataLength;
    if(EFI_ERROR(Status))
    {
        gST->ConOut->OutputString(gST->ConOut,L"Recv: WaitForEvent fail!\n\r");
        Print(L"Status=%x\n", Status);
        return Status;
    }
